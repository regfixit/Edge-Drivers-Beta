...
local function driver_Switched(driver,device)
  ...
  elseif device.preferences.changeProfile == "18006500" and device.preferences.colorTempMinim >= 1800 then
    print("<< rgbw-level-colortemp-1800-6500 >>")
    device:try_update_metadata({profile = "rgbw-level-colortemp-1800-6500"})
...
local function do_added(driver,device)
  if device.network_type == "DEVICE_EDGE_CHILD" then  ---- device (is Child device)
    ...
    print(">>> New Device Joined: " .. lookup_key)
    
    -- 2. CHECK THE MAPPING TABLE
    local target_profile = model_to_profile[lookup_key]
    
    if target_profile then
      if device.active_profile ~= target_profile then
        print(">>> MATCH FOUND: Swapping to " .. target_profile)
        device:try_update_metadata({profile = target_profile})
      else
        print(">>> Profile already matches mapping.")
      end
    else
      print(">>> No specific mapping for this bulb. Using Fingerprint default.")
    end  
    device:refresh()
    --dimmer.do_Preferences (driver, device)
  end
